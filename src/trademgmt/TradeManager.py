...
class TradeManager:
    ...
    def run():
        ...
        TradeManager.intradayTradesDir = os.path.join(
        ...
        if os.path.exists(TradeManager.intradayTradesDir) == False:
            logging.info('TradeManager: Intraday Trades Directory %s does not exist. Hence going to create.',
                         TradeManager.intradayTradesDir)
    ...
    def registerStrategy(strategyInstance):
        TradeManager.strategyToInstanceMap[strategyInstance.getName(
        )] = strategyInstance

    @staticmethod
    def loadAllTradesFromFile():
        tradesFilepath = os.path.join(
            TradeManager.intradayTradesDir, 'trades.json')
        ...
        logging.info('TradeManager: Successfully loaded %d trades from json file %s', len(
            TradeManager.trades), tradesFilepath)
    ...
    def saveAllTradesToFile():
        ...
        logging.info('TradeManager: Saved %d trades to file %s',
                     len(TradeManager.trades), tradesFilepath)
    ...
    def trackSLOrder(trade):
        ...
        if trade.slOrder == None:
            ...
            if trade.slOrder.orderStatus == OrderStatus.COMPLETE:
                ...
                logging.error('SL order %s for tradeID %s cancelled outside of Algo. Setting the trade as completed with exit price as current market price.',
    ...
    def checkAndUpdateTrailSL(trade):
        ...
        if updateSL == True:
            ...
            try:
                ...
                logging.error('TradeManager: Failed to modify SL order for tradeID %s orderId %s: Error => %s',
                              trade.tradeID, trade.slOrder.orderId, str(e))
    ...
    def trackTargetOrder(trade):
        ...
        if trade.targetOrder == None:
            ...
            if trade.targetOrder.orderStatus == OrderStatus.COMPLETE:
                ...
                logging.error('Target order %s for tradeID %s cancelled outside of Algo. Setting the trade as completed with exit price as current market price.',
    ...
    def cancelEntryOrder(trade):
        ...
        try:
            ...
            logging.error('TradeManager: Failed to cancel Entry order %s for tradeID %s: Error => %s',
                          trade.entryOrder.orderId, trade.tradeID, str(e))
        logging.info('TradeManager: Successfully cancelled Entry order %s for tradeID %s',
                     trade.entryOrder.orderId, trade.tradeID)
    ...
    def cancelSLOrder(trade):
        ...
        try:
            ...
            logging.error('TradeManager: Failed to cancel SL order %s for tradeID %s: Error => %s',
                          trade.slOrder.orderId, trade.tradeID, str(e))
    ...
    def cancelTargetOrder(trade):
        ...
        try:
            ...
            logging.error('TradeManager: Failed to cancel Target order %s for tradeID %s: Error => %s',
                          trade.targetOrder.orderId, trade.tradeID, str(e))
    ...
    def squareOffTrade(trade, reason=TradeExitReason.SQUARE_OFF):
        ...
        if trade.targetOrder != None:
            ...
            logging.info('TradeManager: changing target order %s to MARKET to exit position for tradeID %s',
                         trade.targetOrder.orderId, trade.tradeID)
    ...
    def convertJSONToTrade(jsonData):
        ...
        trade.entryOrder = TradeManager.convertJSONToOrder(
            jsonData['entryOrder'])
        trade.slOrder = TradeManager.convertJSONToOrder(jsonData['slOrder'])
        trade.targetOrder = TradeManager.convertJSONToOrder(
            jsonData['targetOrder'])
        return trade
