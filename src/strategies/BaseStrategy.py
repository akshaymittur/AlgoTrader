...
class BaseStrategy:
    def __init__(self, name):
        ...
        self.URL = ''  # URL to be used for this strategy
        self.nifty500_list = pd.read_csv(
            'https://archives.nseindia.com/content/indices/ind_nifty500list.csv')['Symbol'].to_list()  # Nifty 500 list
    ...
    def run(self):
        ...
        if now < self.startTimestamp:
            ...
            logging.info(
                "%s: Waiting for %d seconds till startegy start timestamp reaches...", self.getName(), waitSeconds)
            if waitSeconds > 0:
