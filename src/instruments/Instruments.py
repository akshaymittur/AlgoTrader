...
class Instruments:
    ...
    def loadInstruments():
        ...
        instrumentsFilepath = os.path.join(
            serverConfig['deployDir'], 'instruments.json')
        ...
        logging.info('Instruments: loaded %d instruments from file %s', len(
            instruments), instrumentsFilepath)
        return instruments
    ...
    def fetchInstruments():
        ...
        logging.info(
            'Fetching instruments done. Instruments count = %d', len(instrumentsList))
